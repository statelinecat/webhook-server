...
async def test_webhook(
    ...
    test_data = {
        ...
        "close": {"price": "", "action": "decrease", "decrease": {"type": "posAmountPct", "amount": "1"}, "checkProfit": True},
...
async def get_logs_html(
    ...
    try:
        ...
        html_template = """<!DOCTYPE html><html><head><title>Webhook Logs - {symbol}</title><style>
        ...
        </style></head><body><div class="container"><h2>📊 Webhook Logs: {symbol}</h2>
        <p>🔄 Total instruments: {total_count} | 📋 Showing: {row_count} records</p>
        <table><thead><tr>{headers}</tr></thead><tbody>{rows}</tbody></table></div></body></html>"""
...
async def health_check(queue_manager: QueueManager = Depends(get_queue_manager)):
    placeholder_count = sum(1 for url in webhooks.FINANDY_WEBHOOKS.values() if not webhooks.is_valid_webhook(url))
...
async def get_stats(repository: SignalRepository = Depends(get_repository)):
    try:
        ...
        stats = {"total_signals": len(rows), "status_distribution": {}, "symbol_distribution": {}, "recent_activity": {}}
        ...
        stats["status_distribution"] = dict(sorted(stats["status_distribution"].items(), key=lambda x: x[1], reverse=True))
        stats["symbol_distribution"] = dict(sorted(stats["symbol_distribution"].items(), key=lambda x: x[1], reverse=True)[:10])
